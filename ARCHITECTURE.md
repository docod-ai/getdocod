...
Every reverse_* action (and `extract_from_code`) additionally honors the `reverse_conventions` block in `spec/agent.yaml`: mandatory triangulation of legacy docs (cited as external provenance, never imported), numbered DIV-nn divergences with evidence on both sides, and three question classes never mixed (gap, product decision, external-owner — the last always lands in the single `external-questions` queue, which `status` chases). Three more field-learned blocks in `spec/agent.yaml` bind every producer: `write_order` (stamp `status` LAST — its absence IS the incomplete-run marker, so an interruption cannot pass silently), `reference_discipline` (reference by content and declare in `inputs[]`; file:line evidence carries the observed fragment in backticks on the same line — position AND content, so displacement is provable; naked anchors warn in live documents, snapshot records are exempt; a prose-quoted hash either belongs in `inputs[]` or not in the text) and `divergence_taxonomy` (a FALSE CLAIM is a divergence and justifies an amendment; an UNMET TARGET is the normal condition of a PRD/SLO and justifies nothing — reviewers classify before proposing). Current census: 28 agents. 27 producers plus `tech-lead` (counsel; owns the append-only `counsel` log; the one agent materialized in the main session because sparring is conversation). Identity tests that killed or converted agents in v1: delivers a document (agent) vs. delivers a craft (skill: 12 of them) vs. is a discipline someone obeys (rule: 4 templates specialized by `rules-factory`).

## 3. The artifact registry (`spec/artifacts.yaml`)

Every artifact has: `stage` (metadata, never a folder), `owner` (validator checks the owner agrees), `scope` subset of `[project, ws, target]`, and a `path` template per scope. 38 artifacts (the 38th: `external-questions`, the single append-only queue of questions only an outside owner can close — one file on purpose, unlike the per-agent `decisions` logs, because the queue's value is being ONE list to chase).
