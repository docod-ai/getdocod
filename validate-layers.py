...
def check_generated_refs():
    ...
    # Adapter-level skills may share the docod-* namespace without being
    # subagents (for example Codex's docod-commands router). Derive those
    # names from their own contracts instead of keeping an exception list.
    non_agent_names = set()
    for skill_f in glob.glob(os.path.join(BASE, "adapter-assets", "**", "SKILL.md"), recursive=True):
        skill_body = open(skill_f, encoding="utf-8").read()
        match = re.search(r'^name:\s*docod-([a-z][a-z0-9-]*)\s*$', skill_body, re.MULTILINE)
        if match:
            non_agent_names.add(match.group(1))

    seen = set()
    for rel, body in sources:
        for key in set(re.findall(r'\bdocod-([a-z][a-z0-9-]*)', body)):
            seen.add(key)
            if key not in agents and key not in non_agent_names:
                ERRORS.append(
...
def check_adapter_materialization(adapters):
    """An adapter YAML is not support until something materializes it.

    The Codex adapter field case made the old false-green explicit: capability
    keys could validate 12/12 while install.sh ignored the selected adapter and
    generated only .claude files. This check watches the declaration →
    installer → generated-source chain.
    """
    installer_f = os.path.join(BASE, "install.sh")
    installer = open(installer_f, encoding="utf-8").read() if os.path.exists(installer_f) else ""
    print("\nLAYER 3 · MATERIALIZATION — every adapter has an executor")
    print("-" * 78)

    for f in adapters:
        rel = os.path.relpath(f, BASE)
        ad = yaml.safe_load(open(f, encoding="utf-8")) or {}
        key = (ad.get("adapter") or {}).get("key")
        invoke = ad.get("invoke") or {}
        before = len(ERRORS)
        if not key:
            ERRORS.append(f"[{rel}] adapter.key missing")
            continue
        if not invoke.get("mode"):
            ERRORS.append(f"[{rel}] invoke.mode missing — no invocation contract")
        if key not in installer:
            ERRORS.append(
                f"[{rel}] adapter '{key}' is never named by install.sh — declared support has no materializer"
            )
        for field in ("agentsRoot", "skillsRoot", "command_contracts"):
            value = invoke.get(field)
            if value and value not in installer and value.lstrip("./") not in installer:
                # command_contracts is copied as bundle source rather than
                # generated byte-for-byte; its source must still exist.
                source = value.removeprefix(".docod/").rstrip("/")
                if not os.path.exists(os.path.join(BASE, source)):
                    ERRORS.append(
                        f"[{rel}] invoke.{field} points to '{value}', but neither install.sh nor source materializes it"
                    )

        if key == "codex":
            plugin = os.path.join(BASE, ".codex-plugin", "plugin.json")
            commands = {
                os.path.basename(p)[:-3]
                for p in glob.glob(os.path.join(BASE, "adapter-assets", "codex", "commands", "*.md"))
            }
            skills = {
            ...
            if not os.path.exists(plugin):
                ERRORS.append("[adapters/codex.yaml] native /docod:* plugin manifest missing")
            missing = commands - skills
            if missing:
                ERRORS.append(
                    "[.codex-plugin] command contracts without namespaced skills: " + ", ".join(sorted(missing))
                )
            for skill in sorted(skills):
                policy = os.path.join(BASE, "skills", skill, "agents", "openai.yaml")
                if not os.path.exists(policy):
                    ERRORS.append(
                        f"[skills/{skill}] missing agents/openai.yaml — explicit-only command policy unwatched"
                    )
        print(f"  {'✗' if len(ERRORS) > before else '✓'} {key}")
